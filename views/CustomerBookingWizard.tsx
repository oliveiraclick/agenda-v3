...
const CustomerBookingWizard: React.FC<CustomerBookingWizardProps> = ({ salon, initialServiceId, onLoyalty, onBack }) => {
  ...
  // MUDANÇA: Prioriza nome do perfil conforme solicitado para corrigir exibição
  const establishmentName = salon?.profiles?.name || salon?.name || 'Estabelecimento';
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      if (selectedPro) {
        query = query.eq('professional_name', selectedPro.name);
      }
      ...
      <section className="relative w-full">
        <div className="relative h-[240px] w-full overflow-hidden rounded-b-[2.5rem] shadow-xl">
          ...
          <div className="absolute bottom-0 left-0 w-full p-6">
            ...
            {/* AQUI ESTÁ A CORREÇÃO: Ele tenta pegar o nome do perfil primeiro */}
            <h2 className="text-3xl font-black text-white leading-tight mb-1 tracking-tighter">
              {salon?.profiles?.name || salon?.name || 'Carregando...'}
            </h2>
