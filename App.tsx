...
const App: React.FC = () => {
  ...
  const renderCustomerView = () => {
    switch (customerView) {
      case 'onboarding': return <Onboarding onStart={() => setCustomerView('login')} onLogin={() => setCustomerView('login')} />;
      case 'login': return <AuthLogin onLogin={() => setCustomerView('portal')} onSignup={() => setCustomerView('signup')} onForgot={() => setCustomerView('forgot_password')} onRegisterDetails={() => setCustomerView('auth_register')} />;
      case 'signup': return <AuthSignup onBack={() => setCustomerView('login')} onComplete={() => setCustomerView('portal')} />;
  ...
  return (
    <div className="min-h-screen flex flex-col relative bg-background-light dark:bg-background-dark overflow-x-hidden">
      ...
      {currentRole === 'customer' && !['onboarding', 'login', 'signup', 'forgot_password', 'auth_register'].includes(customerView) && (
